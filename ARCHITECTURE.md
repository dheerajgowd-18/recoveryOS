...
- **Structured Diagnosis Contract (`intelligence/schemas.py`)**: Defines schema-validated diagnosis objects (`StructuredDiagnosis`) with canonical root cause labels (`DiagnosisLabel`), model-reported confidence scores, observable evidence codes, action recommendations, and timing hints.
- **Groq OpenAI-Compatible LLM Provider (`intelligence/providers/groq_provider.py`)**: Uses Groq's OpenAI-compatible endpoint (`https://api.groq.com/openai/v1`) with canonical model `openai/gpt-oss-120b` and structured JSON schema enforcement (`response_format={"type": "json_object"}`) to perform root-cause inference and uncertainty estimation from sanitized observable contexts. Includes live telemetry (`llm_calls`, `llm_successes`, `llm_fallbacks`, `llm_latency_ms`), deterministic fingerprint replay caching for zero-network benchmark reproducibility, and safe fail-closed fallback to the deterministic provider on timeout, schema errors, or missing credentials.
...
- **Candidate Generator (`TimingCandidateGenerator`)**: Generates admissible (Mechanism × Timing Window) candidate pairs bounded by failure physics (e.g. no retries on expired cards, immediate retry only on attempt 1 when allowed, delayed retries for transient gateway timeouts). The LLM cannot reduce the deterministic admissible economic search space by omission; the deterministic system preserves the complete admissible action space for economic evaluation, while LLM reasoning provides ranking, channel, and timing direction preferences.

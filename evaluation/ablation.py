    ...
    def run_ablation(
        ...
        for p_name, r in results.items():
            ...
            summary_dict[p_name] = {
                ...
                "intervention_rate": round(r.metrics.intervention_rate, 3),
                ...
                "diagnosis_accuracy": round(r.metrics.diagnosis_accuracy, 3),
                "fallback_rate": round((r.metrics.diagnosis_fallback_count + r.metrics.strategy_fallback_count) / max(1, r.metrics.total_scenarios), 3),
                "invalid_output_rate": round(r.metrics.invalid_llm_output_count / max(1, r.metrics.total_scenarios), 3),
                "provenance_tags": ["LIVE_LLM" if (r.metrics.diagnosis_live_call_count + r.metrics.strategy_live_call_count > 0) else ("CACHED_LLM" if (r.metrics.diagnosis_cache_hit_count + r.metrics.strategy_cache_hit_count > 0) else "DETERMINISTIC_FALLBACK")],
